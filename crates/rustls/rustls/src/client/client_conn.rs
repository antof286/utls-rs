...
mod connection {
    ...
    impl ClientConnection {
        ...
        pub fn new(config: Arc<ClientConfig>, name: ServerName<'static>) -> Result<Self, Error> {
            Ok(Self {
                inner: ConnectionCore::for_client(config, name, Vec::new(), Protocol::Tcp, None)?.into(),
            })
        }
        ...
        pub fn new_with_override(
            ...
            Ok(Self {
                inner: ConnectionCore::for_client(config, name, Vec::new(), Protocol::Tcp, Some(Random(random)))?.into(),
            })
        }
...
impl ConnectionCore<ClientConnectionData> {
    pub(crate) fn for_client(
        ...
        let state = hs::start_handshake(name, extra_exts, config, &mut cx, random)?;
